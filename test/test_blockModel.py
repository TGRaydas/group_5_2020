...
block_columns = ["id","x", "y","z", "ton", "au", "cat"]
block1_value = ['4', '38', '28', '67', 20.83, 0.2, "p"]
block2_value = ['5', '39', '28', '67', 83.33, 0.3, "p"]
block3_value = ['0', '36', '28', '67', 20.83, 0.2, "c"]
block4_value = ['1', '37', '28', '67', 83.33, 0.3, "p"]
...
class TestBlockModels(TestCase):
    ...
    # Method Tested: find_block_by_index
    # Context: Passing test model blocks
    # Expectation: Should return OK
    def test_find_block_by_index_valid(self):
        block = collection.find_one({"id": "4"})
        test_block = Block(block.keys(), block.values())
        self.assertEqual(test_block.to_json()["id"], test_block_model.find_block_by_index('4', collection).to_json()["id"] )
    
    def test_find_block_by_index_invalid(self):
        self.assertEqual(test_block_model.find_block_by_index(234, collection), False )
    ...
    def test_categorical_attributes(self):
        attribute = "cat"
        bks = [test_block1.to_json(), test_block2.to_json(), test_block3.to_json(), test_block4.to_json()]
        new_value = test_block_model.categorical_attributes(bks, attribute)
        self.assertEqual(new_value, 'p')
...
# Method Tested: delete_block_prec
# Context: passing test model blocks
# should return ok
    def test_delete_block_prec(self):
        delete_test = []
        assert_list = [{'index':"0"},{'index':"1"}]
        delete = test_block_model.delete_block_prec("0", delete_test)
